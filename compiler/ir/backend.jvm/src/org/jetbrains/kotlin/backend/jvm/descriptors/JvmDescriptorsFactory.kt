...
class JvmDescriptorsFactory(
    ...
    private val singletonFieldDescriptors2 = HashMap<IrSymbolOwner, IrField>()
    private val outerThisDescriptors = HashMap<IrClass, IrFieldSymbol>()
    private val outerThisDescriptors2 = HashMap<IrClass, IrField>()
    private val innerClassConstructors = HashMap<IrConstructorSymbol, IrConstructorSymbol>()
    private val innerClassConstructors2 = HashMap<IrConstructor, IrConstructor>()

    override fun getSymbolForEnumEntry(enumEntry: IrEnumEntry): IrField =
        singletonFieldDescriptors2.getOrPut(enumEntry) {
    ...
    override fun getOuterThisFieldSymbol(innerClass: IrClass): IrField =
        if (!innerClass.isInner) throw AssertionError("Class is not inner: ${innerClass.dump()}")
        else outerThisDescriptors2.getOrPut(innerClass) {
            ...
            val symbol = IrFieldSymbolImpl(
                ...
                IrFieldImpl(UNDEFINED_OFFSET, UNDEFINED_OFFSET, DescriptorsFactory.FIELD_FOR_OUTER_THIS, symbol, outerClass.defaultType).also {
    ...
    override fun getInnerClassConstructorWithOuterThisParameter(innerClassConstructor: IrConstructor): IrConstructor {
        ...
        return innerClassConstructors2.getOrPut(innerClassConstructor) {
    ...
    private fun createInnerClassConstructorWithOuterThisParameter(oldConstructor: IrConstructor): IrConstructor {
        ...
        return IrConstructorImpl(oldConstructor.startOffset, oldConstructor.endOffset, oldConstructor.origin, symbol).also {
            newValueParameters.mapIndexedTo(it.valueParameters) { i, v ->
    ...
    override fun getSymbolForObjectInstance(singleton: IrClass): IrField =
        singletonFieldDescriptors2.getOrPut(singleton) {
